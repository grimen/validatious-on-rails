...
** @validates_confirmation_of@ (use this one instead of validates_acceptance_of - it works the same way without Validatious and Rails clash)
...
* Test-suit
* Ruby 1.9.x support
* For the near fanatic Formtastic form-builder users like me: With default config, it's compatible with the Formtastic semantic HTML structure. Note: It seems acceptance_of + check_box not really working so good becuase of the wrapped checkbox-input - Validatious-issue.
